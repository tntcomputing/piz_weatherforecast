...
/* Magic Mirror
 * Module: piz_WeatherForecast
 *
 * By Michael Teeuw http://michaelteeuw.nl
 * MIT Licensed.
 */

Module.register("piz_weatherforecast",{
	...
	// Define required scripts.
	getStyles: function() {
		return ["weather-icons.css", "piz_weatherforecast.css"];
	},
	...
	updateWeather: function() {
		if (this.config.appid === "") {
			Log.error("piz_WeatherForecast: APPID not set!");
			return;
		}
	...
	processWeather: function(data) {
		...
		for (var i in days_forecast) {
			var min = days_forecast[i].temp.reduce(function(previous, current) {if (previous < current) 
																					{return previous;} 
																						else 
																					{return current;}
																			});
			
			//var min = days_forecast[i].temp.reduce(function(previous, current) {current = previous < current ? previous : current});
			var max = days_forecast[i].temp.reduce(function(previous, current) {if(previous > current)
																					{return previous;}
																					else
																					{return current;}
			...
			//var max = days_forecast[i].temp.reduce(function(previous, current) {current = previous > current ? previous : current});
			var _this = this;
			var icon = days_forecast[i].icon.reduce(function(previous, current) 
				{if (_this.config.iconTableOrdered.indexOf(previous) < _this.config.iconTableOrdered.indexOf(current))
					{return previous;}
				else
				{return current;}
			    });
			...
			//var icon = days_forecast[i].icon.reduce(function(previous, current) {current = _this.config.iconTableOrdered.indexOf(previous) < _this.config.iconTableOrdered.indexOf(current) ? previous : current});
